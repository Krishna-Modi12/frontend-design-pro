...
```text
~ [LV-001] page title and section headings are 2px apart — no visual hierarchy   (MEDIUM · critique · hierarchy)
  → b-hierarchy.html:22 — computed h1 font-size 20px vs h2 18px; ratio 1.11
  → suggestion: widen the type ramp (e.g. h1 ~32px, h2 ~20px) so scanning order is legible     [FOUND]
```
