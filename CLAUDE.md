...
A request loads roughly 6,034–7,947 tokens against ~418k of available depth. **Gate 8a hard-fails the build** if any skill exceeds 3,000 tokens alone or 8,000 with deps, so the budget is not advisory. Token count is `file size in bytes ÷ 4`.
...
`.tsx/.ts/.js/.jsx/.html` only** — 418,496 tokens of markdown depth is outside
