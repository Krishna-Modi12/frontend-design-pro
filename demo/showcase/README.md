...
This is one of the two examples in `demo/` that is not a stub-typed reference file — the
other is `landing-page/`, which is also a runnable Next.js app. `dashboard/` and
`auth-form/` are still `.tsx` files compiled only against the ambient `declare module`
stubs in `demo/_stubs.d.ts` — never installed, never run. `demo/showcase/` is the
opposite: a real, standalone Next.js 15 (App Router) +
