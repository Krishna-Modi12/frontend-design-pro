...
5. **Budget: ≤8,000 tokens total.** A typical request lands at 4,775–6,075 — registry + one skill + its declared deps. Over budget: drop the deepest reference first and **say which** in your output.
6. **Load a `skills/{id}/references/*.md` file only when the loaded skill file points you at it** for the specific task. That is where the ~320k tokens of depth lives; none of it is loaded by default.
...
Self-check against `core/validate-checklist.md` before output. **53 machine-enforced constraints (17 parser + 36 regex) plus 4 self-checks.** Fix failures; do not annotate and ship.
