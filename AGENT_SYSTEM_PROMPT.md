...
1. **Read `SKILL.md`** — the registry, ~2,112 tokens. Read it fully. It holds the identity, the anti-slop wall, and the routing table.
...
5. **Budget: ≤8,000 tokens total.** A typical request lands at 5,978–7,598 — registry + one skill + its declared deps. Over budget: drop the deepest reference first and **say which** in your output.
6. **Load a `skills/{id}/references/*.md` file only when the loaded skill file points you at it** for the specific task. That is where the ~376k tokens of depth lives; none of it is loaded by default.
