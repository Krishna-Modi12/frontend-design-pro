...
5. **Budget: ≤8,000 tokens total.** A typical request lands at 5,965–7,530 — registry + one skill + its declared deps. Over budget: drop the deepest reference first and **say which** in your output.
6. **Load a `skills/{id}/references/*.md` file only when the loaded skill file points you at it** for the specific task. That is where the ~372k tokens of depth lives; none of it is loaded by default.
