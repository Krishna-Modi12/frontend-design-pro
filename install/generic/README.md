...
5. Verify: ask which skill it loaded and what that cost. Expect one skill id, its core deps, and a figure in the 5,794–7,394 range. *"I read everything"* means it is not routing.

**Degradation:** depends entirely on file access. Without an on-demand file read, lazy loading becomes retrieval, `@`-referencing or pasting, and the 96 references are out of reach unless you attach them by hand. The per-host matrix is in [docs/AGENT_COMPATIBILITY.md](../../docs/AGENT_COMPATIBILITY.md).
