...
3. Narrow integration: add the one or two `skills/{id}/SKILL.md` files plus the `core/*.md` deps you know you need. Broad integration: add all 8 `core/*.md` and all 17 skill routers — small enough that a large context window absorbs it. Leave `references/` out; 320,375 tokens is too large to paste wholesale in any window.
...
**Degradation:** static context by default, not lazy loading — whatever sits in the system instruction is paid on every request, needed or not. A large context window makes that affordable, not free. Only a function-calling loop you build yourself gets the ~4,800–6,100-tokens-per-request model.
