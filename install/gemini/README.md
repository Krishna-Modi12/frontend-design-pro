...
3. Narrow integration: add the one or two `skills/{id}/SKILL.md` files plus the `core/*.md` deps you know you need. Broad integration: add all 8 `core/*.md` and all 17 skill routers — small enough that a large context window absorbs it. Leave `references/` out; 320,865 tokens is too large to paste wholesale in any window.
