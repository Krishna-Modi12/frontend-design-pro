...
- 43 examples compile under `tsc --noEmit` strict + noImplicitAny
...
- 8 blocking gates, and the archive is unzipped and re-verified against its
...
11 skills → 15 skills grew the always-loaded registry by 296 tokens.

Marginal cost of a new skill: ~43 tokens of permanent context.
Depth is free because it's lazy.

4/ Quality is machine-enforced, not asserted.

8 blocking gates. 43 examples compile under tsc strict. 16 semantic
...
8 blocking gates in `scripts/build_release.py`, ~45 seconds:

1. Pre-flight — token ceiling, version consistency across three files
2. Frontmatter — 15/15 skills declare deps that exist
