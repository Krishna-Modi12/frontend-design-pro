...
Ask **"which skill did you load?"** — you want one skill id and a token figure around 4,800–6,100, not a claim that it read the whole pack. If it cannot name the skill, the rule is not attaching: check that the file is under `.cursor/rules/`, has the `.mdc` extension, and that your `globs` match the file you have open.
