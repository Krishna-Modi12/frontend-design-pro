...
- **The 20-file cap forces a subset.** 147 routable files in the pack (the root `SKILL.md`, 8 `core/`, 19 skill routers, 119 references), 20 slots. You are curating in advance exactly the decision the registry was designed to defer until the request arrives. Pick for the work you actually do; a GPT scoped to "landing pages and design systems" beats one that tries to cover all 19 skills.
