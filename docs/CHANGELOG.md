...
- The showcase is **not** exempt from the content rules: the regex suite judges it as a fourth demo project, and the parser AST checks run on all seven of its components. Only the compile regime differs.
- Two real defects surfaced this way: `BentoGrid.tsx` had a `transition-opacity duration-300` with no `motion-reduce` escape (MOTION-01), and `Footer.tsx` declared no types at all (TS-01-AST). Both fixed.
