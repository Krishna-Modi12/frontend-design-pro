...
**Claude Code is the only host with a real filesystem for that**, so it's the only one with true lazy loading — `SKILL.md`, the one matched skill, and its declared `core/*.md` deps load automatically per request (~6,034–7,947 tokens), leaving the other 422,220 tokens of reference depth untouched until the routing table points at something.
...
A: Yes. `SKILL.md` carries the identity, behavioural preamble, anti-slop wall, the 19-row routing table and failure handling — it's a complete, self-contained registry at 2,146 tokens, and no system-prompt setup is required to use it (`README.md`, [`INSTALL.md`](INSTALL.md)).
