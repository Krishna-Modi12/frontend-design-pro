...
**Claude Code is the only host with a real filesystem for that**, so it's the only one with true lazy loading — `SKILL.md`, the one matched skill, and its declared `core/*.md` deps load automatically per request (~6,037–7,950 tokens), leaving the other 433,477 tokens of reference depth untouched until the routing table points at something.
