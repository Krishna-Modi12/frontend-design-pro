...
**Claude Code is the only host with a real filesystem for that**, so it's the only one with true lazy loading — `SKILL.md`, the one matched skill, and its declared `core/*.md` deps load automatically per request (~5,999–7,598 tokens), leaving the other 402,592 tokens of reference depth untouched until the routing table points at something.
