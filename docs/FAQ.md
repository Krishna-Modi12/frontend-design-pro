...
`demo/landing-page/` sits on the same side of that line as `showcase` — its own `package.json`, real dependencies, and a dev server that boots (`cd demo/landing-page && npm install && npm run dev`). It is excluded from `demo/tsconfig.json` for the same reason `showcase` is, and type-checks against its real vendor typings instead.
