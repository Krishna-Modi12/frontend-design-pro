...
- **No built-in awareness of the gate scripts.** The API has no concept of `scripts/build_release.py` or the AST/regex constraint suites unless you wire them in as tools yourself (e.g., a tool that shells out to run the parser constraints against generated code and returns pass/fail). Without that, "51 machine-enforced constraints" only holds if you run them outside the model.
