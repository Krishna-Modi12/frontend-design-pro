...
| `skills/{id}/references/*.md` | 104 deep references | **357,580 tokens** | only when a skill file points at one for the task at hand |
...
**Ceiling is 7,476 tokens against 357,580 available.** Gate 8a fails the build if any skill exceeds 3,000 tokens alone or 8,000 with dependencies, so this cannot silently regress.
...
references are ~358k tokens and are the part an agent actually opens for depth,
