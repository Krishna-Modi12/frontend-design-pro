...
| `skills/{id}/references/*.md` | 118 deep references | **425,658 tokens** | only when a skill file points at one for the task at hand |
...
**Ceiling is 7,950 tokens against 425,658 available.** Gate 8a fails the build if any skill exceeds 3,000 tokens alone or 8,000 with dependencies, so this cannot silently regress.
...
| 10 | References | the 19 ban-shaped constraints, run over every fenced `tsx`/`jsx`/`ts`/`js`/`html` block in all 118 references, 19 skill routers and 8 core files | 122 files · 0 violations |
...
Gates 3–5 judge `skills/*/examples/*.tsx` and `demo/` — 55 files. The 118
references are ~426k tokens and are the part an agent actually opens for depth,
