...
|---|---|---|---|
| `SKILL.md` | Identity, behavioural preamble, anti-slop wall, 19-row routing table, loading protocol, failure table | **2,112 tokens** | always |
...
| `skills/{id}/references/*.md` | 105 deep references | **373,600 tokens** | only when a skill file points at one for the task at hand |
...
**Ceiling is 7,476 tokens against 373,600 available.** Gate 8a fails the build if any skill exceeds 3,000 tokens alone or 8,000 with dependencies, so this cannot silently regress.
...
| 10 | References | the 19 ban-shaped constraints, run over every fenced `tsx`/`jsx`/`ts`/`js`/`html` block in all 105 references, 19 skill routers and 8 core files | 122 files · 0 violations |
...
Gates 3–5 judge `skills/*/examples/*.tsx` and `demo/` — 55 files. The 105
references are ~374k tokens and are the part an agent actually opens for depth,
...
**`design-system/references/brand-design-systems.md` was orphaned** — present on disk, absent from its skill's Reference Index, so nothing could route to it. A citation was added; 76/105 references now resolve with none orphaned.
