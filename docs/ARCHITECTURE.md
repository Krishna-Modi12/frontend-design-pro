...
| `skills/{id}/references/*.md` | 117 deep references | **421,374 tokens** | only when a skill file points at one for the task at hand |
...
**Ceiling is 7,947 tokens against 421,374 available.** Gate 8a fails the build if any skill exceeds 3,000 tokens alone or 8,000 with dependencies, so this cannot silently regress.
