...
| `skills/{id}/references/*.md` | 99 deep references | **349,118 tokens** | only when a skill file points at one for the task at hand |
...
**Ceiling is 7,476 tokens against 349,118 available.** Gate 8a fails the build if any skill exceeds 3,000 tokens alone or 8,000 with dependencies, so this cannot silently regress.
