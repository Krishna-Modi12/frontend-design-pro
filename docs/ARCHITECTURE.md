...
| `skills/{id}/references/*.md` | 112 deep references | **404,917 tokens** | only when a skill file points at one for the task at hand |
...
**Ceiling is 7,927 tokens against 404,917 available.** Gate 8a fails the build if any skill exceeds 3,000 tokens alone or 8,000 with dependencies, so this cannot silently regress.
...
Gates 3–5 judge `skills/*/examples/*.tsx` and `demo/` — 55 files. The 112
references are ~405k tokens and are the part an agent actually opens for depth,
