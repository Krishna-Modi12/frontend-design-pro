...
| `skills/{id}/references/*.md` | 94 deep references | **339,838 tokens** | only when a skill file points at one for the task at hand |
...
**Ceiling is 7,266 tokens against 339,838 available.** Gate 8a fails the build if any skill exceeds 3,000 tokens alone or 8,000 with dependencies, so this cannot silently regress.
