...
| `skills/{id}/references/*.md` | 99 deep references | **348,105 tokens** | only when a skill file points at one for the task at hand |
...
**Ceiling is 7,476 tokens against 348,105 available.** Gate 8a fails the build if any skill exceeds 3,000 tokens alone or 8,000 with dependencies, so this cannot silently regress.
...
| 10 | References | the 19 ban-shaped constraints, run over every fenced `tsx`/`jsx`/`ts`/`js`/`html` block in all 101 references, 19 skill routers and 8 core files | 122 files · 0 violations |
...
**`design-system/references/brand-design-systems.md` was orphaned** — present on disk, absent from its skill's Reference Index, so nothing could route to it. A citation was added; 76/101 references now resolve with none orphaned.
