...
|---|---|---|---|
| [`demo/landing-page/`](demo/landing-page/) | **Runnable Next.js app** | `landing-pages` + `core/design-tokens.md` | Dark OKLCH surface, asymmetric bento features, tabular-nums metric strip, skip link — serves its own `/api/site/overview` |
...
The two stub-typed demos are checked by the same suites as the gold examples — `tsc --noEmit` strict, 17 AST constraints, 36 regex constraints:
...
`landing-page` reads its metric strip and price book from `/api/site/overview`. That endpoint belongs to the fictional product it advertises, so the demo serves it itself from [`demo/landing-page/screenshot-fixture.json`](demo/landing-page/screenshot-fixture.json) — committed so the capture above is reproducible rather than asserted, and so the page's loading, error and empty states are reachable by editing one file instead of by faking a timer.
...
[`demo/showcase/`](demo/showcase/) is the second project in `demo/` to break the stub-typed convention above on purpose, and the more ambitious of the two: a real, standalone Next.js 15 + React 19 + Tailwind v4 app — its own `package.json`, real installed dependencies (React Three Fiber + drei, React Hook Form + Zod), a dev server that actually boots. It's a cinematic dark-mode landing page for a fictional AI analytics product, "Nexus" — near-black OKLCH surface, single acid-green accent, an asymmetric bento grid, a WebGL particle hero, and a validated contact form.
