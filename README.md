...
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **19** | **100** | **346,279 tokens** | **2,099 tokens** | **5,912–7,476** | **59** | **11** |
...
|---|---|---|---|
| [`demo/landing-page/`](demo/landing-page/) | **Runnable Next.js app** | `landing-pages` + `core/design-tokens.md` | Light editorial page for a fictional product — 7:5 serif hero beside a rendered product report, tabular-nums metric strip, asymmetric bento, testimonials |
...
**[`demo/landing-page/`](demo/landing-page/)** — a launch page for "Bellwether", a fictional schema-migration rehearsal tool. Light OKLCH surface at a single neutral hue, one deep cobalt accent, a variable editorial serif driven by its own optical-size axis, tabular-nums metric strip. **[Open it live →](https://krishna-modi12.github.io/frontend-design-pro/landing-page/)**

![Bellwether landing page — a light porcelain surface with a deep cobalt accent, an editorial serif hero reading "Every migration runs twice. The first time, nothing is at stake.", and beside it a rehearsal report card listing rows touched, traffic replayed and lock duration, an hourly write-volume chart marking the recommended window, and a hold verdict in prose](demo/landing-page/screenshot.png)
...
That first instruction is the point of this demo. The anti-slop wall bans three AI-design defaults *unless the brief asks for them*, and the brief behind the previous version of this page asked for one by name — "near-black background, acid-green accent". Nothing was violated and the result still looked generated, because an exemption had been taken rather than a decision made. This brief declines it, and the palette that replaced it was chosen against measured contrast on two grounds rather than by eye.
...
**The route it takes.** *Report*, *severity*, *dark* and *contrast* match the trigger-keyword column, so the registry loads `design-principles` for the information hierarchy, `design-system` for the OKLCH token pair, and `web-interface` for the copy rules — each pulling `core/design-tokens.md`, plus the two universal deps. Roughly 5,900 tokens against 346,279 available.
...
|---|---|---|
| **frontend-design-pro** | 59 (17 AST + 42 regex) | **Yes** — Gate 10 runs 19 ban-shaped constraints over all 100 references and blocks the archive |
